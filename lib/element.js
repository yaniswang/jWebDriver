...
Element.prototype = {
	...
	_init: function(browser, using, value){
		...
		if(value === undefined){
			...
			var result = browser.doCommand('getElement', {'using': using,'value': value});
			self._id = browser.isOk(result) ? result.ELEMENT : result;
		}
	},
	...
	find: function(using, value){
		...
		var result = self._doElementCommand('findElement', {'using': using,'value': value});
		if(self.browser.isOk(result)){
			return new Element(self.browser, 'element', result.ELEMENT);
		}
		return result;
	},
