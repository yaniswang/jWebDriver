...
const Browser = PromiseClass.create({
    ...
    init(done){
        ...
        new Promise(function(resolve){
            ...
            driver.exec('newSession', {}, sessionOptions, function(error, ret){
                if(error){
                    done(error);
                }
                else{
    ...
    url(url, done){
        ...
        // get url
            ...
            self.exec('getUrl', function(error, ret){
    ...
    eval(script, args, done){
        ...
        if(typeof script === 'string' || typeof script === 'function'){
            ...
            self.exec(isAsync?'evalASync':'eval', {}, {
                script: script,
                args: args
            }, function(error, ret){
    ...
    windowHandle(useCache, done){
        ...
        if(useCache && windowHandle){
            ...
            self.exec('getCurrentWindowHandle', function(error, ret){
                if(ret){
     ...
     */
    windowHandles(done){
        this.exec('getAllWindowHandles', function(error, ret){
            done(error, ret && ret.value);
        });
    },
    ...
    switchFrameParent(done){
        this.exec('switchFrameParent', done);
    },
    ...
    position(x, y, done){
        ...
        Promise.resolve(self.windowHandle(true)).then(function(windowHandle){
            if(x === undefined){
                return self.exec('getWindowPosition', {
    ...
    size(width, height, done){
        ...
        Promise.resolve(self.windowHandle(true)).then(function(windowHandle){
            if(width === undefined){
                return self.exec('getWindowSize', {
                    windowHandle: windowHandle
                }, function(error, ret){
                    done(error, ret && ret.value);
                });
    ...
    getScreenshot(filename, done){
        ...
        this.exec('getScreenshot', function(error, ret){
     ...
     */
    title(done){
        this.exec('getTitle', function(error, ret){
            done(error, ret && ret.value);
        });
    },
     ...
     */
    source(done){
        this.exec('getSource', function(error, ret){
            done(error, ret && ret.value);
        });
    },
    ...
    cookies(isArray, done){
        ...
        this.exec('getAllCookies', function(error, ret){
            if(error){
                done(error);
            }
            else{
     ...
     */
    clearCookies(done){
        this.exec('clearAllCookies', done);
    },
    ...
    localStorageKeys(done){
        this.exec('getAllLocalStorageKeys', function(error, ret){
            done(error, ret && ret.value);
        });
    },
    ...
    localStorage(name, value, done){
        ...
        if(value !== undefined){
            self.exec('setLocalStorage', {}, {
                key: String(name),
                value: String(value)
            }, done);
        }
        else{
            self.exec('getLocalStorage', {
                key: String(name)
            }, function(error, ret){
                done(error, ret && ret.value);
            });
    ...
    clearLocalStorages(done){
        this.exec('clearAllLocalStorages', done);
    },
    ...
    sessionStorageKeys(done){
        this.exec('getAllSessionStorageKeys', function(error, ret){
            done(error, ret && ret.value);
        });
    },
    ...
    sessionStorage(name, value, done){
        ...
        if(value !== undefined){
            self.exec('setSessionStorage', {}, {
                key: String(name),
                value: String(value)
            }, done);
        }
        else{
            self.exec('getSessionStorage', {
                key: String(name)
            }, function(error, ret){
                done(error, ret && ret.value);
            });
    ...
    removeSessionStorage(name, done){
        this.exec('deleteSessionStorage', {
            key: name
        }, done);
    },
    ...
    clearSessionStorages(done){
        this.exec('clearAllSessionStorages', done);
    },
     ...
     */
    getAlert(done){
        this.exec('getAlert', function(error, ret){
            done(error, ret && ret.value);
        });
    },
    ...
    findVisible(using, value, done){
        ...
        self.find(using, value, function*(error, elements){
            if(elements.length > 0){
                ...
                let newJson = [];
                try{
                    newJson = yield self.eval('function(arg1){\
    ...
    wait(using, value, options, done){
        ...
        function waitElement(){
            self[(self.browserMode && displayed)?'findVisible':'find'](using, value, function(error, elements){
                if(elements){
                ...
                if(new Date().getTime() - startTime > timeout){
                    let message = 'Wait elelment '+(removed?'removed':'displayed')+' timeout: '+using+' ,'+value+' ,' + timeout + 'ms';
                    self.log('ERROR', message);
                    endWait(message);
                }
                else{
    ...
    orientation(orientation, done){
        ...
        if(orientation){
            ...
            self.exec('getOrientation', function(error, ret){
                done(error, ret && ret.value);
            });
        }
    },
    ...
    geolocation(latitude, longitude, altitude, done){
        ...
        if(latitude !== undefined){
            ...
            self.exec('getLocation', function(error, ret){
                done(error, ret && ret.value);
            });
        }
    },
    ...
    close(done){
        let self = this;
        self.exec('delSession', function(error){
     ...
     */
    contexts(done){
        this.exec('getContexts', function(error, ret){
            done(error, ret && ret.value);
        });
    },
    ...
    context(contextId, done){
        ...
        self.exec(contextId ? 'setContext' : 'getContext', {}, {
            name: contextId
        }, function(error, ret){
            done(error, ret && ret.value);
        });
     ...
     * @param  {Function} done callback function
     */
    touchSwipe(startX, startY, endX, endY, duration, done){
        if(typeof duration === 'function'){
            duration = 20;
        }
        done = getDone(arguments);
        this.exec('touchSwipe', {}, {
            startX: startX,
            startY: startY,
            endX: endX,
            endY: endY,
