...
const Browser = PromiseClass.create({
     ...
     */
    addLog(type, message){
        this._driver.addLog(type, message);
    },
     ...
     */
    logs(type, done){
        this.execCmd('getLogs', {}, {type: type}, function(error, ret){
            done(error, ret && ret.value);
        });
    },

    /**
     * get webdriver log types
     * @method logTypes
     * @public
     * @return {Array} typs
     */
    logTypes(done){
        this.execCmd('getLogTypes', function(error, ret){
            done(error, ret && ret.value);
        });
    },
            ...
            if(png64){
                if(elem){
                    ...
                    if(elems.length === 1){
                        ...
                        png64 = yield new Promise((resolve,reject) => gmShot.toBuffer('PNG', (error, buffer) => error?reject(error): resolve(buffer)));
